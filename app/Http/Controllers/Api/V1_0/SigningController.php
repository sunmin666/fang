...
{
	/**
	 * @apiDefine Groupsigning 签约
	 */

	/**
	 * @api        {get} api/1.0.0/signing/create  签约发起
	 * @apiName    create
	 * @apiGroup   Groupsigning
	 *
	 * @apiParam (参数) {string} buyid 认购id
	 *
	 * @apiSampleRequest http://192.168.1.13/fang/public/api/1.0.0/signing/create
	 * @apiVersion 1.0.0
	...
	public function signing_create()
	{
		...
		//查询缴费信息
		$data -> buyid = $buyid;
		$data -> dingjin =  Signing::get_payloginfo($data -> buy_number);
		if($data -> pay_type == 0){
			$data -> month_pay  = '';
			$data -> loan_term  = '';
		}
	...
	/**
	 * @api        {post} api/1.0.0/signing/store  签约新增
	 * @apiName    store
	 * @apiGroup   Groupsigning
	 *
	 ...
	 * @apiParam (参数) {string} buyid 认购id
	 *
	 * @apiSampleRequest http://192.168.1.13/fang/public/api/1.0.0/signing/store
	 * @apiVersion 1.0.0
	 * @apiSuccessExample {json} 成功返回:
	 *     HTTP/1.1 200 OK
	 *     {
	...
	public function signing_store(Request $api){
	...
	/**
	 * @api        {get} api/1.0.0/signing/search  签约列表与检索
	 * @apiName    search
	 * @apiGroup   Groupsigning
	 *
	 ...
	 *
	 * @apiSampleRequest http://192.168.1.13/fang/public/api/1.0.0/signing/search
	 * @apiVersion 1.0.0
	 * @apiSuccessExample {json} 成功返回:
	 *     HTTP/1.1 200 OK
	 *     {
	...
	public function signing_search(){
		...
		$data = Signing::get_signing( $cust_id , $page );
		if($data){
			return response()->json( [
				'code'    => '101' ,
				'message' => '请求成功' ,
	...
	/**
	 * @api        {get} api/1.0.0/signing/view  签约详情
	 * @apiName    view
	 * @apiGroup   Groupsigning
	 *
	 * @apiParam (参数) {string} signid 签约id
	 *
	 * @apiSampleRequest http://192.168.1.13/fang/public/api/1.0.0/signing/view
	 * @apiVersion 1.0.0
	 * @apiSuccessExample {json} 成功返回:
	 *     HTTP/1.1 200 OK
	 *     {
	...
	public function signing_view(){
		...
		$data = Signing::get_d_signing($sig_id);

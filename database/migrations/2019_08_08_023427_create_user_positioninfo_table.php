...
class CreateUserPositioninfoTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('user_positioninfo', function (Blueprint $table) {
					...
					//职位的详情描述
					$table -> string('description','300') -> nullable(FALSE) ->comment('职位描述');
					//添加时间
					$table -> string('created_at',11) -> nullable(FALSE) -> comment('添加的时间戳');
					//修改时间
					$table -> string('updated_at',11) -> nullable(TRUE) -> comment('修改时间戳');
        });
			DB::statement( "ALTER TABLE `user_positioninfo` comment '用户职位信息表'" );
    }
    ...
    {
        Schema::dropIfExists('user_positioninfo');
    }
}

            ...
            <form role="form" action="#" method="post" name="reg_memberinfo" id="reg_memberinfo">
                ...
                <div class="form-group">
					...
					<textarea name="content" id="remarks"
                              cols="30" rows="5" class="form-control" style=" resize: none;" placeholder="{{trans('delegate.text4')}}" ></textarea>
                </div>
...
//        var cust = $( '#cust' ).val();    //客户   *
        ...
        $.ajax( {
            ...
            success : function ( data ) {
                //console.log(data);
                if ( data.code == {{config('myconfig.delegate.delegate_store_error_code')}}) {
                    layer.msg( data.msg , { time : 2151 } );
                    $( "#store1" ).attr( 'disabled' , false );
                    return false;
                }

                if ( data.code == {{config('myconfig.delegate.delegate_store_success_code')}}) {

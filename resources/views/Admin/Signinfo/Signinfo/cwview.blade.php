...
<script>
	...
	$( '#store1' ).click( function () {
		...
		$.ajax( {
			url : "{{URL('signinfo/update_cwview')}}" ,
			type : 'post' ,
			data : {
				sigid:"{{$sigid}}",
				buyid:"{{$buyid}}",
			...
			success : function ( data ) {
				...
				if ( data.code == {{config('myconfig.signinfo.cwview_sigs_success_code')}}) {
					layer.msg( data.msg , { time : 2000 } , function () {
						window.parent.location.reload();
					} );
				}

				if ( data.code == {{config('myconfig.signinfo.cwview_sig_error_code')}}) {
					layer.msg( data.msg , { time : 2000 } );
					$( "#store1" ).attr( 'disabled' , false );
				}
				if ( data.code == {{config('myconfig.signinfo.cwview_sig_success_code')}}) {
					layer.msg( data.msg , { time : 2000 },function(){
						window.parent.location.reload();
					});

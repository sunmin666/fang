...
<script>
	...
	$( '#store1' ).click( function () {
		...
		$.ajax( {
			url : "{{URL('signinfo/update_review')}}" ,
			type : 'post' ,
			data : {
				sigid:"{{$sigid}}",
				buyid:"{{$buyid}}",
				sign_status:manager_verify_status,
			...
			success : function ( data ) {
				...
				if ( data.code == {{config('myconfig.signinfo.review_sigs_success_code')}}) {
					layer.msg( data.msg , { time : 2000 } , function () {
						window.parent.location.reload();
					} );
				}

				if ( data.code == {{config('myconfig.signinfo.review_sig_error_code')}}) {
					layer.msg( data.msg , { time : 2000 } );
					$( "#store1" ).attr( 'disabled' , false );
				}
				if ( data.code == {{config('myconfig.signinfo.review_sig_success_code')}}) {
					layer.msg( data.msg , { time : 2000 },function(){
						window.parent.location.reload();
					} );

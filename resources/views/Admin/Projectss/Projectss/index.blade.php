...
@section('content')
	...
	<div class="box box-info">
			...
			<div class="table-responsive">
				<table class="table no-margin">
					...
					<tr>
						...
						{{--项目中文名称--}}
						<th>{{ trans('project.pro_cname') }}</th>
						{{--项目英文名称--}}
						<th>{{ trans('project.pro_ename') }}</th>
						{{--项目所属公司--}}
						<th>{{ trans('project.comp_id') }}</th>
						{{--项目录入时间--}}
						<th>{{ trans('project.created_at') }}</th>
						{{--操作--}}
						<th>{{ trans('project.operating') }}</th>
					</tr>
					</thead>
					<tbody>
					@foreach($project as $value)
						<tr>
							...
							<td>
								{{--<button type="button" value="{{$value -> project_id}}"--}}
								        {{--onclick="status({{$value -> comp_id}},{{$value -> project_id}})"--}}
									...
									{{--@endif--}}
								{{--</button>--}}
								{{--<button type="button" value="{{$value -> project_id}}" onclick="view({{$value -> project_id}})"--}}
								        {{--class="btn btn-warning btn-xs btn_edit" id="btn_edit"><i--}}
										{{--class="fa fa-edit"></i> {{trans('memberinfo.news_view')}}</button>--}}
								<button type="button" value="{{$value -> project_id}}" onclick="edit({{$value -> project_id}})"
								        class="btn btn-warning btn-xs btn_edit" id="btn_edit"><i
										class="fa fa-edit"></i> {{trans('memberinfo.news_edits')}}</button>
								<button type="button" value="{{$value -> project_id}}" onclick="d({{$value -> project_id}})"
								        class="btn btn-warning btn-xs btn_delete"><i
		...
		<div class="box-footer clearfix">
			...
			<div class=" pull-right">{{$project -> links()}}</div>
			<input type="hidden" value="{{$project -> count()}}" id="page_count">
		</div>
	</div>
@endsection
@push('include-js')
	...
	<script type="text/javascript">
			...
			layer.confirm( "{{trans('permission.is_delete_info')}}" , {
				...
				$.ajax( {
					...
					success : function ( data ) {
						console.log( data );
						if ( data.code == {{config('myconfig.project.delete_success_project_code')}} ) {
							layer.msg( data.msg , { time : 2000 } , function () {
								if ( page_count == vote.length ) {
									location.href = "{{URL('projectss/19')}}";
								}
								else {
									window.location.reload();
								}
						...
						else if ( data.code == {{config('myconfig.project.delete_error_project_code')}} ) {
		...
		//删除信息
		function d( project_id ) {
			var page_count = $( '#page_count' ).val();
			layer.confirm( "{{trans('memberinfo.is_delete_info')}}" , {
				btn : ["{{trans('memberinfo.confirm')}}" , "{{trans('memberinfo.cancel')}}"] //按钮
			} , function () {
				$.post( "{{URL('projectss/destroy')}}" , { 'project_id' : project_id , '_token' : "{{csrf_token()}}" } ,
					function ( data ) {
						console.log( data );
						if ( data.code == {{config('myconfig.project.delete_error_project_code')}} ) {
							layer.msg( data.msg , { time : 2000 } );
						}
						if ( data.code == {{config('myconfig.project.delete_success_project_code')}} ) {
		...
		function edit( project_id ) {
			layer.open( {
				...
				shadeClose : false ,
				shade : 0.5 ,
				content : ["{{URL('projectss/edit')}}" + "/" + project_id] ,
				success : function ( layero , index ) {
					$( ':focus' ).blur();
				}
			} );

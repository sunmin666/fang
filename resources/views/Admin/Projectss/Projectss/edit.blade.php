		...
		<div class="box-body">
			...
			<form role="form" action="#" method="post" name="reg_memberinfo" id="reg_memberinfo">
				<div class="form-group">
					...
					<input type="text" class="form-control" value="{{$project -> pro_cname}}" name="pro_cname" placeholder="请填写项目中文名称" id="pro_cname"
					       maxlength="50">
				</div>
				{{--项目英文名称--}}
				<div class="form-group">
					<label>{{ trans('project.pro_ename') }}：</label>
					<input type="text" class="form-control" value="{{$project -> pro_ename}}" maxlength="100" placeholder="请填写项目英文名称"
				...
				<div class="form-group">
					...
					<select name="comp_id" id="comp_id" class="form-control">
						...
						{{--@foreach($company as $k => $v)--}}
							{{--<option value="{{$v -> comp_id}}" @if($v -> comp_id == $project -> comp_id) selected @endif> {{$v -> comp_cname}}</option>--}}
						{{--@endforeach--}}
					</select>
	...
	$( '#store1' ).click( function () {
		...
		$.ajax( {
			...
			success : function ( data ) {
				...
				if ( data.code == {{config('myconfig.project.update_success_project_code')}}) {
					layer.msg( data.msg , { time : 2000 } , function () {
						window.parent.location.reload();
					} );
				}

				if ( data.code == {{config('myconfig.project.update_error_project_code')}}) {

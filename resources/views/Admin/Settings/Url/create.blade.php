		...
		<div class="box-body">
			...
			<form role="form" action="#" method="post" name="reg_memberinfo" id="reg_memberinfo" onkeydown="if(event.keyCode==13){return false;}">
				<div class="form-group">
					...
					<input type="text" class="form-control" name="url_name" id="url_name"
				...
				<div class="form-group">
					...
					<textarea name="description" class="form-control" id="description" onkeyup="value=value.replace(/[\d]/g,'') "
					          placeholder="请输入url描述" maxlength="255" style="resize:none" cols="15" rows="5"></textarea>
				</div>
				{{ csrf_field()}}
			</form>
		</div>
	...
	$( '#store1' ).click( function () {
		...
		$.ajax( {
			...
			success : function ( data ) {
				...
				if ( data.code == {{config('myconfig.url.url_store_success_code')}}) {
					layer.msg( data.msg , { time : 2000 } , function () {
						window.parent.location.reload();
					} );
				}

				if ( data.code == {{config('myconfig.url.url_store_error_code')}}) {

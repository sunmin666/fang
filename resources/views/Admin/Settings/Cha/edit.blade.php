		...
		<div class="box-body">
			...
			<form role="form" action="#" method="post" name="reg_memberinfo" id="reg_memberinfo">
				<div class="form-group">
					...
					<input type="text" class="form-control" value="{{$cha-> ch_nsme}}" name="ch_nsme" id="ch_nsme" maxlength="10">
				</div>
				<div class="form-group">
					<label>{{ trans('cha.ch_text') }}：</label>
					<input type="text" class="form-control" maxlength="30"
					       value="{{$cha-> ch_text}}" name="ch_text" id="ch_text">
				</div>
				<input type="hidden" value="{{$cha -> chid}}" id="chid">
				<div class="form-group">
					<label>{{ trans('cha.ch_per') }}：</label><br>
					<div style="padding-left: 30px">
						@foreach($pers as $k => $v)
							<input type="checkbox" class="i-checks" name="groupCheckbox[]"
							       @foreach($cha -> ch_per as $kr => $vr)
							        @if($v -> perid == $vr)
								       checked
								      @endif
							       @endforeach
							...
							@foreach($v -> xsuperior as $k1 => $v1)
								<input type="checkbox" class="i-checks fuxuan{{$v -> perid}}"  name="groupCheckbox[]"
								       @foreach($cha -> ch_per as $kr => $vr)
								       @if($v1 -> perid == $vr)
								       checked
									     @endif
...
<script>
	...
	$(document).ready(function(){
	 // var ccc =	$('input[name="groupCheckbox[]"]:checked').val();
		var vote = [];
		for ( var i = 0 ; i < $( 'input[name="groupCheckbox[]"]:checked').length ; i++ ) {
			if ( $( 'input[name="groupCheckbox[]"]:checked' ).eq( i ).prop( "checked" ) ) {
				vote.push( $( 'input[name="groupCheckbox[]"]:checked' ).eq( i ).val() )
			}
		}

		for(var ig = 0; ig < vote.length; ig++ ){
			$('.fuxuan'+ vote[ig]).attr('disabled',false);
		}
	});
	...
	$( '#store1' ).click( function () {
		...
		$.ajax( {
			...
			success : function ( data ) {
				...
				if ( data.code == {{config('myconfig.cha.ch_update_success_code')}}) {
				...
				if ( data.code == {{config('myconfig.cha.ch_update_error_code')}}) {
					layer.msg( data.msg , { time : 2000 } );
					$( "#store1" ).attr( 'disabled' , false );
				}
			}

			...
			<form role="form" action="#" method="post" name="reg_memberinfo" id="reg_memberinfo" onkeydown="if(event.keyCode==13){return false;}">
				<div class="form-group">
					...
					<input type="text" class="form-control" name="role_title" id="role_title"
					       maxlength="20" value="{{$role -> role_title}}" placeholder="请输入角色标题" onkeyup="value=value.replace(/[^\w\.\/]/ig,'')">
				</div>
				<div class="form-group">
					<label>{{ trans('role.role_name') }}：</label>
					<input type="text" class="form-control" maxlength="50"
					       name="role_name" value="{{$role -> role_name}}" id="role_name" placeholder="请输入角色名称" onkeyup="this.value=this.value.replace(/[^\u4e00-\u9fa5]/g,'')">
				</div>
				<div class="form-group">
					...
					<textarea name="description" class="form-control" id="description" onkeyup="this.value=this.value.replace(/[^\u4e00-\u9fa5]/g,'')"
					          placeholder="请输入角色描述"  maxlength="255" style="resize:none" cols="15" rows="5">{{$role -> description}}</textarea>
				</div>
				{{ csrf_field()}}
			</form>
		</div>
	...
	$( '#store1' ).click( function () {
		...
		$.ajax( {
			...
			success : function ( data ) {
				...
				if ( data.code == {{config('myconfig.role.update_role_success_code')}}) {
				...
				if ( data.code == {{config('myconfig.role.update_role_error_code')}}) {
					layer.msg( data.msg , { time : 2000 } );
					$( "#store1" ).attr( 'disabled' , false );
				}
			}

		...
		<div class="box-body">
			...
			<form role="form" action="#" method="post" name="reg_memberinfo" id="reg_memberinfo" onkeydown="if(event.keyCode==13){return false;}">
				<div class="form-group">
					...
					<input type="text" class="form-control" name="perm_title" id="perm_title" value="{{$permin ->perm_title }}"
				...
				<div class="form-group">
					...
					<select name="role_id" id="role_id" class="form-control">
						...
						@foreach($role as $k => $v)
							<option value="{{$v -> role_id}}" @if($v -> role_id == $permin -> role_id ) selected @endif>{{$v -> role_name}}</option>
						@endforeach
					</select>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<textarea name="description" class="form-control" id="description" onkeyup="value=value.replace(/[^\w\u4E00-\u9FA5]/g, '')"
					          placeholder="请输入权限描述" maxlength="255" style="resize:none" cols="15" rows="5">{{$permin -> description}}</textarea>
				</div>
				<div class="form-group">
						...
						@foreach($url as $k => $v)
							<div style="float: left;width: 25%;height: 30px" >
								<input type="checkbox" class="i-checks" name="groupCheckbox[]"
								       @foreach($permin -> http_path as $k3 => $v3) @if($v3 == $v -> url_id) checked @endif @endforeach
...
<script>
	...
	$( '#store1' ).click( function () {
		...
		$.ajax( {
			url : "{{URL('permin/update')}}" ,
			type : 'post' ,
			data : {
				permin_id:"{{$permin -> perm_id}}",
			...
			success : function ( data ) {
				...
				if ( data.code == {{config('myconfig.permin.permin_update_success_code')}}) {
					layer.msg( data.msg , { time : 2000 } , function () {
						window.parent.location.reload();
					} );
				}
				if ( data.code == {{config('myconfig.permin.permin_update_error_code')}}) {

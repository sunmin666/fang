...
@section('content')
	...
	<div class="box box-info">
			...
			<div class="table-responsive">
				<table class="table no-margin">
					...
					<tr>
						...
						{{--职位标题--}}
						<th>{{ trans('position.posi_title') }}</th>
						{{-- 职位名称--}}
						<th>{{ trans('position.posi_name') }}</th>
						{{--职位描述--}}
						<th>{{ trans('position.description') }}</th>
						{{--添加时间--}}
						<th>{{ trans('position.created_at') }}</th>
					...
					@foreach($position as $value)
						<tr>
							<td><input type="checkbox" class="i-checks" id="groupCheckbox" name="groupCheckbox[]"
							           value="{{$value->posi_id}}"
								></td>
							<td>{{ $value -> posi_title}}</td>
							<td>{{$value -> posi_name}}</td>
							<td>{{$value -> description}}</td>
							<td>{{date('Y-m-d H:s',$value -> created_at)}}</td>
							<td>
								<button type="button" value="{{$value -> posi_id}}" onclick="edit({{$value -> posi_id}})"
								        class="btn btn-warning btn-xs btn_edit" id="btn_edit"><i
										class="fa fa-edit"></i> {{trans('memberinfo.news_edits')}}</button>
								<button type="button" value="{{$value -> posi_id}}"
								        onclick="d({{$value -> posi_id}})"
		...
		<div class="box-footer clearfix">
			...
			<div class=" pull-right">{{$position -> links()}}</div>
			<input type="hidden" value="{{$position -> count()}}" id="page_count">
		</div>
	</div>
@endsection
@push('include-js')
	...
	<script type="text/javascript">
		...
		$( '.select_all' ).click( function () {
			...
			layer.confirm( "{{trans('permission.is_delete_info')}}" , {
				...
				$.ajax( {
					url : '{{URL('positioninfo/destroy_all')}}' ,
					type : 'post' ,
					data : {
						'posi_id' : vote ,
						'_token' : "{{csrf_token()}}"
					...
					success : function ( data ) {
						console.log( data );
						if ( data.code == {{config('myconfig.position.delete_success_position_code')}} ) {
							layer.msg( data.msg , { time : 2000 } , function () {
								if ( page_count == vote.length ) {
									location.href = "{{URL('position/2')}}";
								}
								else {
									window.location.reload();
								}
						...
						else if ( data.code == {{config('myconfig.position.delete_error_position_code')}} ) {
		...
		$( '#news_day' ).click( function () {
			layer.open( {
				...
				shadeClose : false ,
				shade : 0.5 ,
				content : ["{{URL('positioninfo/create')}}"] ,
		...
		function d( posi_id ) {
			...
			layer.confirm( "{{trans('permission.is_delete_info')}}" , {
				...
				$.post( "{{URL('positioninfo/destroy')}}" , { 'posi_id' : posi_id , '_token' : "{{csrf_token()}}" } ,
					function ( data ) {
						console.log( data );

						if ( data.code == {{config('myconfig.position.delete_error_position_code')}}) {
							layer.msg( data.msg , { time : 1523 } );
						}
						else if ( data.code == {{config('myconfig.position.delete_success_position_code')}} ) {
		...
		function edit( posi_id ) {
			layer.open( {
				...
				shadeClose : false ,
				shade : 0.5 ,
				content : ["{{URL('positioninfo/edit')}}" + "/" + posi_id] ,
				success : function ( layero , index ) {
					$( ':focus' ).blur();
				}
			} );

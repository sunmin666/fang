			...
			<form role="form" action="#" method="post" name="reg_memberinfo" id="reg_memberinfo" onkeydown="if(event.keyCode==13){return false;}">
				<div class="form-group">
					...
					<input type="text" class="form-control" name="posi_title" id="posi_title"
					       maxlength="20" placeholder="请输入职位标题" onkeyup="value=value.replace(/[^\w\.\/]/ig,'')">
				</div>
				<div class="form-group">
					<label>{{ trans('position.posi_name') }}：</label>
					<input type="text" class="form-control" maxlength="50"
					       name="posi_name" id="posi_name" placeholder="请输入职位名称" onkeyup="this.value=this.value.replace(/[^\u4e00-\u9fa5]/g,'')">
				</div>
				<div class="form-group">
					...
					<textarea name="description" class="form-control" id="description" onkeyup="this.value=this.value.replace(/[^\u4e00-\u9fa5]/g,'')"
					          placeholder="请输入职位描述" maxlength="255" style="resize:none" cols="15" rows="5"></textarea>
				</div>
	...
	$( '#store1' ).click( function () {
		...
		$.ajax( {
			...
			success : function ( data ) {
				...
				if ( data.code == {{config('myconfig.position.success_position_store_code')}}) {
				...
				if ( data.code == {{config('myconfig.position.error_position_store_code')}}) {
					layer.msg( data.msg , { time : 2000 } );
					$( "#store1" ).attr( 'disabled' , false );
				}

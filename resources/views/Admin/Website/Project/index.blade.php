		...
		<div class="box-body">
			<div class="table-responsive">
				<table class="table no-margin">
					...
					<tr>
						<th>{{ trans('pro.nid') }}</th>
						<th>{{ trans('pro.n_title') }}</th>
						<th>{{ trans('pro.n_img') }}</th>
						<th>{{ trans('pro.created_at') }}</th>
						<th>{{ trans('pro.n_admin_at') }}</th>
						<th>{{ trans('pro.operating') }}</th>
					</tr>
					</thead>
					<tbody>
	...
	<script type="text/javascript">
		...
		function d( nid ) {
			...
			layer.confirm( "{{trans('permission.is_delete_info')}}" , {
				...
				$.post( "{{URL('pro/destroy')}}" , { 'nid' : nid , '_token' : "{{csrf_token()}}" } ,
					function ( data ) {
						console.log(data);

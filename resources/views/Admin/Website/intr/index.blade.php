			...
			<div class="table-responsive">
				<table class="table no-margin">
					...
					<tr>
						...
						<th>{{ trans('intr.i_title') }}</th>
						<th>{{ trans('intr.created_at') }}</th>
						<th>{{ trans('intr.operating') }}</th>
					</tr>
					</thead>
					<tbody>
	...
	<script type="text/javascript">
						...
						if ( data.code == {{config('myconfig.intr.intr_delete_success_code')}} ) {
							layer.msg( data.msg , { time : 2000 } , function () {
								if ( page_count == vote.length ) {
									location.href = "{{URL('introduction/6')}}";
								}
								else {
									window.location.reload();
								}
		...
		function d( intr_id ) {
			...
			layer.confirm( "{{trans('permission.is_delete_info')}}" , {
				...
				$.post( "{{URL('intr/destroy')}}" , { 'intr_id' : intr_id , '_token' : "{{csrf_token()}}" } ,
					function ( data ) {
						console.log( data );

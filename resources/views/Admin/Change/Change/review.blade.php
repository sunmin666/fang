...
<script>
	...
	$( '#store1' ).click( function () {
		...
		$.ajax( {
			url : "{{URL('change_home/update_review')}}" ,
			type : 'post' ,
			data : {
				chan_id:  "{{$chan_id}}",
				new_homeid:"{{$new_homeid}}",
				status:manager_verify_status,
				verify_remarks:manager_verify_remarks,
				_token : "{{csrf_token()}}"
			} ,
			success : function ( data ) {
				console.log( data );

				if ( data.code == {{config('myconfig.buy.buy_review_success_code')}}) {
					layer.msg( data.msg , { time : 2000 } , function () {
						window.parent.location.reload();
					} );
				}

				if ( data.code == {{config('myconfig.buy.buy_review_error_code')}}) {
					layer.msg( data.msg , { time : 2000 } );
					$( "#store1" ).attr( 'disabled' , false );
				}
				if ( data.code == {{config('myconfig.buy.buy_review_successe_code')}}) {
					layer.msg( data.msg , { time : 2000 },function(){
						window.parent.location.reload();
					} );

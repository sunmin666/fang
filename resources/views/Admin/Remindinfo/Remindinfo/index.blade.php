...
@section('content')
    ...
    <div class="box box-info">
            ...
            <div class="table-responsive">
                <table class="table no-margin">
                    ...
                    <tr>
                        ...
                        {{--职业顾问--}}
                        <th>{{ trans('remindinfo.hous') }}</th>
                        {{--模块--}}
                        <th>{{ trans('remindinfo.modular') }}</th>
                        {{--提醒内容--}}
                        <th>{{ trans('remindinfo.content') }}</th>
                         {{--提醒时间--}}
                         <th>{{ trans('remindinfo.remind_time') }}</th>
                        {{--添加时间--}}
                        <th>{{ trans('remindinfo.created_at') }}</th>
                         {{--更新时间--}}
                         <th>{{ trans('remindinfo.updated_at') }}</th>
                        {{--操作--}}
                    ...
                    @foreach($remind as $key=> $value)
                        <tr>
                            ...
                            <td>{{$value -> name}}</td>
                            <td>{{ $value -> content}}</td>
                            <td>@if($value ->modular ==1 )
                                    客户跟踪
                                @elseif($value ->modular ==2 )
                                    客户来访
                                @elseif($value ->modular ==3 )
                                    缴费记录
                                @elseif($value ->modular ==4 )
                                    认购签约
                                @elseif($value ->modular ==5 )
                                    更名
                                @elseif($value ->modular ==6 )
                                    换房
                                @elseif($value ->modular ==7 )
                                    退房
                                @else
                                    延迟签约
                                @endif
                            </td>
                            <td>{{date('Y-m-d H:i',$value -> remind_time)}}</td>
                            <td>{{date('Y-m-d H:i',$value -> created_at)}}</td>
                            <td>@if($value -> updated_at == '')暂无更新@else{{date('Y-m-d H:i',$value -> updated_at)}}@endif</td>
                            <td>
                                <button type="button" value="{{$value->remi_id}}" onclick="view({{$value->remi_id}})"
                                        class="btn btn-warning btn-xs btn_edit" id="btn_edit"><i
                                            class="fa fa-edit"></i> {{trans('memberinfo.news_view')}}</button>
                                <button type="button" value="{{$value->remi_id}}" onclick="edit({{$value->remi_id}})"
                                        class="btn btn-warning btn-xs btn_edit" id="btn_edit"><i
                                            class="fa fa-edit"></i> {{trans('memberinfo.news_edits')}}</button>
                                <button type="button" value="{{$value->remi_id}}" onclick="d({{$value->remi_id}})"
        ...
        <div class="box-footer clearfix">
            ...
            <div class=" pull-right">{{$remind -> links()}}</div>
            <input type="hidden" value="{{$remind -> count()}}" id="page_count">
        </div>
    </div>
    @endsection
    @push('include-js')
    ...
    <script type="text/javascript">
        ...
        $( '.select_all' ).click( function () {
            ...
            layer.confirm( "{{trans('permission.is_delete_info')}}" , {
                ...
                $.ajax( {
                    url : '{{URL('remindinfo/destroy_all')}}' ,
                    type : 'post' ,
                    data : {
                        'c_id' : vote,
                        '_token' : "{{csrf_token()}}"
                    ...
                    success : function ( data ) {
                        console.log( data );
                        if ( data.code == {{config('myconfig.remindinfo.remindinfo_del_success_code')}} ) {
                            layer.msg( data.msg , { time : 2000 } , function () {
                                if ( page_count == vote.length ) {
                                    location.href = "{{URL('remindinfo/64')}}";
                                }
                                else {
                                    window.location.reload();
                                }
                        ...
                        else if ( data.code == {{config('myconfig.remindinfo.remindinfo_del_error_code')}} ) {
        ...
        $( '#news_day' ).click( function () {
            layer.open( {
                ...
                shadeClose : false ,
                shade : 0.5 ,
                content : ["{{URL('remindinfoddd/create')}}"] ,
        ...
        function d( remi_id ) {
            ...
            layer.confirm( "{{trans('memberinfo.is_delete_info')}}" , {
                ...
                $.post( "{{URL('remindinfo/del')}}" , { 'remi_id' : remi_id , '_token' : "{{csrf_token()}}" } ,
                        function ( data ) {
                            console.log( data );
                            if ( data.code == {{config('myconfig.remindinfo.remindinfo_del_error_code')}} ) {
                                layer.msg( data.msg , { time : 2000 } );
                            }
                            if ( data.code == {{config('myconfig.remindinfo.remindinfo_del_success_code')}} ) {
        ...
        function edit( remi_id ) {
            layer.open( {
                ...
                shadeClose : false ,
                shade : 0.5 ,
                content : ["{{URL('remindinfo/edit')}}" + "/" + remi_id] ,
                success : function ( layero , index ) {
                    $( ':focus' ).blur();
                }
            } );
        ...
        function view( remi_id ) {
            layer.open( {
                ...
                shadeClose : false ,
                shade : 0.5 ,
                content : ["{{URL('remindinfo/view')}}" + "/" + remi_id] ,
                success : function ( layero , index ) {
                    $( ':focus' ).blur();
                }
            } );

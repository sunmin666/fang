			...
			<form role="form" action="#" method="post" name="reg_memberinfo" id="reg_memberinfo">
				...
				<div class="form-group">
					...
					<textarea name="remarks" id="remarks"
					          cols="30" rows="5" class="form-control" placeholder="{{trans('trackinfo.text4')}}" style="resize:none">{{$checkout -> remarks}}</textarea>
				</div>
			</form>
		</div>
	</div>
...
<script>
	...
	$( '#store1' ).click( function () {
		...
		$.ajax( {
			...
			type : 'post' ,
			data : {
				chan_id:"{{$checkout -> chan_id}}",
				remarks : remarks ,           //备注
				_token : "{{csrf_token()}}"             //post提交token验证
			} ,
			success : function ( data ) {
				console.log( data );
				if ( data.code == {{config('myconfig.checkout.update_error_chang_code')}}) {
					layer.msg( data.msg , { time : 2151 } );
					$( "#store1" ).attr( 'disabled' , false );
					return false;
				}

				if ( data.code == {{config('myconfig.checkout.update_success_chang_code')}}) {

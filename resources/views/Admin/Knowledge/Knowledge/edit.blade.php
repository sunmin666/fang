			...
			<form role="form" action="#" method="post" name="reg_memberinfo" id="reg_memberinfo">
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="title"
					       placeholder="请输入标题" id="title" value="{{$data -> title}}"
					       maxlength="15">
				</div>
				{{--单元号--}}
				<div class="form-group">
					...
					<select name="class_id" id="class_id" class="form-control">
						...
						@foreach($Knowledge as $k => $v)
							<option value="{{$v ->field_id}}" @if($v ->field_id == $data -> class_id) selected @endif>{{$v -> name}}</option>
						@endforeach
					</select>
				</div>
				{{--房号--}}
				...
				<video src="{{URL::asset('uploads/shipin/'.$data->video)}}" controls="controls" id="myvideo" class="none" style="width: 50%;height:300px" ></video>

				<input type="hidden" id="video" value="{{$data -> video }}">

				{{--楼层--}}
				<div class="form-group">
					<label>{{ trans('knowledge.content') }}：</label>
					<div>
						<textarea id="summernote">{{$data-> content}}</textarea>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<select name="is_public" id="is_public" class="form-control">
						<option value="">--请选择--</option>
						<option value="0" @if($data->is_public == 0) selected @endif>公开</option>
						<option value="1" @if($data->is_public == 1) selected @endif>不公开</option>
					</select>
				</div>
...
<script>
	...
	$( '#store1' ).click( function () {
		...
		$.ajax( {
			...
			success : function ( data ) {
				console.log( data );
				if ( data.code == {{config('myconfig.knowledge.update_k_error_code')}}) {
					layer.msg( data.msg , { time : 2151 } );
					$( "#store1" ).attr( 'disabled' , false );
					return false;
				}

				if ( data.code == {{config('myconfig.knowledge.update_k_success_code')}}) {
